...
//---------------------------------------------------------------------------------------------------------------
bool Graph::disconnect(const string &from, const string &to) const {
  // check if its in the list
...
Graph::dijkstra(const string &startLabel) {
  ...
  priority_queue<pair<int, string>, vector<pair<int, string>>,
      ...
      pq;
  if (!adjacencyList.count(startLabel)) {
    return make_pair(weights, previous);
  }

  for (const auto &vertex : adjacencyList) {
    // this is basically infinite bc we dont know the weight of getting to
    // that
    // vertex (INT MAX is essent)
    weights[vertex.first] = INT16_MAX;
    previous[vertex.first] = "";
  }
  weights[startLabel] = 0;
  pq.push(make_pair(0, startLabel));

  // this is where we will visit the vertices in the graph in order of weight
  // (lo to hi)
  while (!pq.empty()) {
    int currentWeight = pq.top().first;
    string currentLabel = pq.top().second;
    pq.pop();
    // here we are checking for the shorter path, if the current weight is
    // greater than what the weight to that vertex is, we skip it;
    if (currentWeight > weights[currentLabel]) {
      continue;
    }
    ...
    for (const auto &neighbor : adjacencyList.at(currentLabel)) {
      ...
      int newEdgeWeight = currentWeight + neighborWeight;
      if (newEdgeWeight < weights[neighborLabel]) {
        weights[neighborLabel] = newEdgeWeight;
        previous[neighborLabel] = currentLabel;
        pq.push(make_pair(newEdgeWeight, neighborLabel));
      }
    }
  }
